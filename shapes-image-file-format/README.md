...
- **It does not beat WebP either, once WebP is configured the way anyone would ship it.** The claimed 1–6% win below ~29.2 dB was measured against `cwebp` on its *default* `-m 4`; at `-m 6` the win becomes a wash that alternates sign between adjacent samples, and the curves meet at ~26.1 dB instead (report 06 #9). At **3840×2160** even the wash is gone: the deficit grows monotonically with resolution, reaching **+19.3%** at the eval's own fidelity (report 08). **That figure has since been superseded — see report 16.** It described a coder whose wall coster was not decodable and which had no cross-channel transform; with both fixed, the deficit at matched fidelity is **+0.930%** — a **real file**, not an estimate: 132,301 B against WebP's 131,082 B at 28.5 dB, round-tripping bit-exactly (report 21). **But that is one photograph, and it is an outlier.** Across the full **Kodak-24** set the same pipeline is **+3.6% to +71.5%, mean +27.8%, with zero wins** (report 23). It is not a resolution effect — the same wallpaper at Kodak's own 768×512 is +1.3%. Two candidate predictors of when the format is competitive were tested and both failed, so **its applicable domain is uncharacterised.**

- **But against the baseline that matches what it delivers — WebP *plus a region-map sidecar* — it wins on every image measured.** Kodak-24: **24 of 24, mean +30.5%, range +18.7% to +37.8%** (report 24), plus +44.1% on the Sierra wallpaper. The format is a poor image codec and a good structured-image format: if you want pixels use WebP; if you want pixels **and** a segmentation, this is the cheapest way measured to get both, by roughly 30%. **This is the first claim in the study to survive a corpus rather than rest on one image.** This still appears to be the first matched-fidelity measurement of a region coder against a modern codec; the classical "geometry wins at low bitrate" literature benchmarks against JPEG-2000, not VP8/AV1-era codecs. *(That novelty claim is unverified — see the caveat in report 04.)*

- **There is no file size at which WebP has no answer — but at the very bottom of the axis the two are level.** At 3840×2160 `cwebp` bottoms out at `q0` = 85,102 B, which briefly looked like a capability the region coder had and WebP did not. It is not one: encoding at 960×540 and upscaling — what every responsive-image pipeline already does, and still 3840×2160 pixels on screen — puts a **20,066 B** WebP at **24.54 dB** where the shape coder's *native 4K* encode manages only 21.99 dB (report 06 #10). But the same knob belongs to both sides, and when the shape coder is also allowed to encode small and upscale it reaches **24.59 dB at 20,618 B** — a tie. The published −2.55 dB at that rung was an artifact of steelmanning one side only, and is retracted as #11. Above the floor, where both coders work at native resolution, WebP leads by **0.67 to 3.20 dB** and the deficit widens with rate.

- **At lossless it slips under AVIF and PNG, and still loses to WebP and JPEG XL.** The exact region partition of a 4K photograph costs **11,713,104 B** against WebP-lossless's 7,718,506 (**1.52×**) and JPEG XL's 6,468,598 (**1.81×**), while coming in just under AVIF's 11,969,137 and 4.6% under PNG's 12,278,280. The partition degenerates to 1.305 px per region, the walls become nearly free at 0.45 bits/edge, and **93% of the bill is colour** — at which point the region coder simply *is* a raster coder with one predictor where WebP has fourteen. *(This row read 12,159,385 B until 2026-07-29, when it turned out to be priced with a weaker colour predictor than the rest of its own table — ledger item B6.)*
...
| 06 | [`06-corrections-and-falsifications.md`](06-corrections-and-falsifications.md) | The **thirteen** claims this investigation produced and then killed, and the measurement that killed each |
...
| 21 | [`21-the-container.md`](21-the-container.md) | **SHPC v1 — the format is a file.** ~20 B of container overhead, bit-exact round trip; parity stops being a claim |
| 20 | [`20-…`](06-corrections-and-falsifications.md) | *(P3 — the wall coder made legal; recorded in report 06 #12 and the ledger)* |
| 19 | [`19-sidecar-baseline.md`](19-sidecar-baseline.md) | Against **WebP + a region-map sidecar** — the baseline that matches the product — **40–44% smaller** |
| 18 | [`18-encoder-cost.md`](18-encoder-cost.md) | Encode is **3 m 44 s / 2.89 GB** at 4K, single-threaded. Rules out on-demand pipelines, not the ranked applications |
| 17 | [`17-capability-operating-point.md`](17-capability-operating-point.md) | At the rate where the segmentation is good, a **dead heat** — and the first attempt made falsification #11 again |
| 16 | [`16-matched-fidelity.md`](16-matched-fidelity.md) | **At matched fidelity the gap is 0.91%**, not 19.3% — after fixing the wall coder's schedule and giving the colour coder the transform WebP has had since 2010 |
| 15 | [`15-recolour.md`](15-recolour.md) | The colour column re-priced: −9% to −36%, and a modelled number that points the **wrong way** at four operating points |
| 14 | [`14-are-the-regions-meaningful.md`](14-are-the-regions-meaningful.md) | The regions **are** meaningful — but the capability sweet spot is 227–1,383 regions, not the 11,121 the byte work optimised |
| 13 | [`13-what-this-format-is-for.md`](13-what-this-format-is-for.md) | Strengths, applications and weaknesses; the baseline should be WebP **plus a region-map sidecar** |
| 09 | [`09-wall-coder-levers.md`](09-wall-coder-levers.md) | The component every earlier report assumed was mature: the crack coder is **not decodable**, and reordering its two planes is worth **12.6%** of the wall bill for no extra contexts |

Data companions: [`03-corpus-dictionary-data.txt`](03-corpus-dictionary-data.txt), [`04-region-merge-frontier-data.txt`](04-region-merge-frontier-data.txt), [`05-codec-rd-sweep-data.txt`](05-codec-rd-sweep-data.txt), [`05-codec-rd-sweep-strong-data.txt`](05-codec-rd-sweep-strong-data.txt), [`05-matched-fidelity-comparison-data.txt`](05-matched-fidelity-comparison-data.txt), [`08-native-resolution-data.txt`](08-native-resolution-data.txt), [`08-resolution-ladder-data.txt`](08-resolution-ladder-data.txt), [`08-rate-ladder-data.txt`](08-rate-ladder-data.txt), [`08-rate-floor-steelman-data.txt`](08-rate-floor-steelman-data.txt), [`08-window-fidelity-data.txt`](08-window-fidelity-data.txt), [`09-crossplane-wall-data.txt`](09-crossplane-wall-data.txt), [`09-context-width-data.txt`](09-context-width-data.txt).
