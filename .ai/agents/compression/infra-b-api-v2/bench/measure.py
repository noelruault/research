...
Usage:
  measure.py <algo> <corpus_dir> <out_items.json> [DICT=path env]
  RUNS=N env override (default 30)
  WARMUP=N env override (default 3)
"""
...
def measure_item(item_path, encode_cmd, decode_cmd, runs=30, warmup=3):
    raw = Path(item_path).read_bytes()
    enc_us, wire_bytes, encoded = time_one(encode_cmd, stdin_bytes=raw, runs=runs, warmup=warmup)
    dec_us, _,          _       = time_one(decode_cmd, stdin_bytes=encoded, runs=runs, warmup=warmup)
    return {
...
def main():
    """Usage: measure.py <algo> <corpus_dir> <out_items.json> [DICT env] [RUNS env]"""
    ...
    for p in sorted(Path(corpus_dir).iterdir()):
        ...
        items[p.name] = measure_item(p, enc, dec, runs=runs, warmup=warmup)
