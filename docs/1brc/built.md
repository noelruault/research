...
- env-baseline 0ddbb3f — physical floor measured: 754.4 ms +/- 8.8 ms to read 13.8 GB (15 uncached preads), mmap 5-9x slower than read(), page cache slower than F_NOCACHE; also closed `asm-recon` 82b1a0a (top-five technique inventory, six hypotheses, NEON movemask and merykitty's parse both verified in Go) and `go-skeleton` ba75f9d (1brc/code/go module, byte-compare gate green on 413 and 10k stations, hyperfine harness, 26.1 ns/row)
