...
**The wrong half: syscall count — KILLED-on-argument, with the number.** v1 issues one `pread` per 4 MiB buffer: 13,795,610,267 bytes / 4 MiB is about **3,289 reads for the whole file** [CORRECTED, C8: the default is now 1 MiB (E-24), so it is **13,157 reads**, 4.0x more — the kill holds a fortiori, since the argument is that even thousands of reads are negligible against a 754 ms floor, and E-24 measured the smaller buffer FASTER while making four times as many calls], roughly 220 per worker. Even at 10 µs each that is ~33 ms of syscall entry spread across 15 cores against a 754 ms I/O floor. There is no room in that budget for batching to matter, and darwin has no `io_uring` to batch with anyway. The argument does not expire: it is arithmetic over a recorded byte count and a recorded buffer size, and shrinking the syscall count can only ever recover a fraction of a number that is already small.
...
**Superseded by measurement, and not a correction: the assumption held and the number moved because the binary got faster.** E-20 measured the idle gap with `-phases` and it *is* the read (29-30% of worker wall blocked in `pread`, against shard skew under 4% and a merge under 0.4%), so the assumption this paragraph flagged as unmeasured is now measured and stands. The ceiling itself is recomputed on the baseline of the day — 18.53 s of CPU over 15 cores is 1.235 s against a 1.50 s wall — which is **17.6%, not 25%** (**and 19.1% on E-25's baseline, 1.152 s against a 1.424 s wall; then 20.4% on E-27's, 0.9812 s against a 1.233 s wall, which is the first baseline on which this ceiling lands UNDER the 1.000 s target rather than over it; the ledger's board carries the current figure**), because E-17's oversubscription already took some of the gap this figure was reserving. The ledger's queue item 14 and its board row carry the measured number; this paragraph keeps the original so the arithmetic that produced 25% stays readable. **E-29 built the mechanism and killed it: +0.57%/+1.57% of wall, overlapping both bracket slots. The hedge in the sentence above turns out to have been the whole answer — oversubscription did not take "some" of the gap, it took all of it, and no ceiling on this line was ever reachable. See `CORRECTIONS.md` C10.**
...
**Verdict: QUEUED as H-14**, and it is the largest single-mechanism candidate in this report. [**Closed at E-29: built, measured, KILLED-on-numbers. It was the largest candidate this pass produced and it returned nothing, which is the transfer pass's most expensive miss and the one worth reading.**]
...
|---|---|---|---|---|
| 1 | **H-14** double-buffered workers | DPDK's fill-while-you-process | 5% to a 25% ceiling [**re-derived per baseline: 17.6% at E-20, 19.1% at E-25, 20.4% at E-27; then MEASURED at E-29 and KILLED — +0.57%/+1.57% of wall, overlapping both bracket slots, C10**] | nothing |
