...
## 0. v3 lineage (lean cherry-pick from v2)

This is v3. It is v1 plus exactly three changes from v2, chosen because they fix real
defects observed in actual runs. Everything else in v2 (Phase 0.5 Tools check, Phase
0.6 Calibration, Phase 8 Manifest, §5.0 SCOPE schema validator, §11.1 findings path
resolver, DISCARD-BY-PREDICTION template, strict items.json schema validation) was
ceremony that did not change any decision and is **deliberately omitted**.

### What v3 keeps from v2

1. **`bench/measure.py` canonical sub-millisecond CPU timer (Section 4.5).** v1 used
   `hyperfine --shell=none "sh -c '...'"` which adds ~6 ms of subprocess startup per
   measurement. That floor saturated all encode/decode work below it and produced the
   wrong winner on Infra B (zstd-dict-3 picked when zstd-dict-9 was correct). v3
   replaces the sub-ms timing path with a Python `subprocess.run` + `time.perf_counter_ns`
   measurer. `hyperfine` is still used for inputs ≥50 KB or expected per-iteration cost
   ≥5 ms, where its overhead is negligible.

2. **§2.5 Brotli static-dictionary mismatch warning on JSON corpora.** Brotli's RFC 7932
   Appendix A 120 KB static dictionary is HTML/JS-tuned. On JSON, log lines, telemetry,
   and other non-web-text payloads, low-quality Brotli (`brotli -1`) can produce more
   wire bytes than `gzip -6`. v1's table did not flag this; v3 does, with the v1 Infra-B
   numbers as the citation.

3. **`score.py` dual CI output (Section 4.4).** Reading "[CI -129275, -26591]" without
   scale was hard. v3's `score.py` outputs both absolute byte deltas AND percentage
   deltas relative to baseline, plus a per-item breakdown.

### What v3 rejects from v2

| v2 addition | Reason it was dropped |
|---|---|
| Phase 0.5 Tooling check | A one-line `command -v X` inline is sufficient. The schema-of-tools file is overhead; missing tools surface naturally when an experiment fails. |
| Phase 0.6 Local CPU calibration | The §2.1 table is orientative. The real calibration is the bench itself, which is run anyway. The 5-second sanity bench changed no decision in the v2 test run. |
| Phase 8 Manifest write | `bench/manifest.json` is useful for cross-session diff awareness, but most invocations are one-shot. When reproducibility across machines matters, capture it then; do not pay the cost every session. |
...
| §11.1 findings path resolver | The path bug is fixed at the prompt boundary: callers state the absolute output path, the agent obeys. ~30 lines of bash ceremony for what one line of prompt accomplishes. |
| DISCARD-BY-PREDICTION template | Prose with a §3 citation works identically. |
| Strict `items.json` schema validation | Define a working shape, do not impose validation. Tools change output formats; rigid validators break with them. |
| Renaming the 8-phase loop to 10-phase | Cosmetic. |

### Decision rule unchanged

The autoresearch loop, security gates, citation discipline, and decision rule (KEEP iff
bootstrap CI95 of the score delta is strictly negative) are unchanged from v1.
...
### 4.4 The metric in code (v3: dual-format CI)

Emit `bench/score.py` once. v3 changes from v1: outputs both absolute byte-deltas AND
percentage deltas relative to baseline, plus a per-item array. Reading
"[CI -129275, -26591]" without scale was hard; v3 fixes that.

```python
#!/usr/bin/env python3
"""Bootstrap-CI scorer for compression experiments. v3: dual-format CI."""
...
### 4.5 Canonical sub-millisecond CPU timer (v3)
...
<50 KB or encoders/decoders that finish in <5 ms. v1 used the `hyperfine -- sh -c`
path for everything and that produced the wrong winner on a JSON API workload (zstd
level 3 picked when level 9 was correct, because both encode times saturated to the
shell-startup floor and the score formula could not differentiate them).

v3 emits a Python timer that fork/execs the encoder directly via `subprocess.run` and
times with `time.perf_counter_ns`. **Use this for everything below the 5 ms / 50 KB
...
```python
#!/usr/bin/env python3
"""Canonical sub-millisecond CPU timer for compression experiments. v3.
