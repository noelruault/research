...
IDLE_MIN="${IDLE_MIN:-0}"
# awk must DRAIN ioreg's 438 KB rather than exit at the first match: an early close SIGPIPEs ioreg, and under `set -euo pipefail` the 141 propagates and kills the caller silently (measured 296/300 calls).
