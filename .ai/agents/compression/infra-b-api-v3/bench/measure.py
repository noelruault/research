#!/usr/bin/env python3
"""Canonical sub-millisecond CPU timer for compression experiments. v3 (Section 4.5).
...
This is the v3 fix vs v1: v1 used `hyperfine -- sh -c '...'` which adds
~6 ms of subprocess startup floor per measurement, saturating sub-ms encode/decode
work and producing the wrong winner on JSON API workloads. v3 forks the encoder
directly via subprocess.run + time.perf_counter_ns.
"""
