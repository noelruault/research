...
| ~~P-06b~~ | ~~Push `bitsPerEdge` above 1.73~~ | **CLOSED — report 27. Provably inert**: scaling both constants cannot change merge ordering, and a 73% rise gives a byte-identical render. The ratio, the real parameter, slides along the RD curve without improving it — **no arm dominates the base**. The whole P-06 family is closed |
