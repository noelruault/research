...
def main(baseline_path, candidate_path, weights, exclusions=None):
    base = json.loads(Path(baseline_path).read_text())
    cand = json.loads(Path(candidate_path).read_text())
    excl = set(exclusions or [])
    keys = [k for k in base if k in cand and k not in excl]
    ...
    out = {
        ...
        "baseline_total_wire_bytes": sum(base[k]["wire_bytes_p95"] for k in keys),
        "candidate_total_wire_bytes": sum(cand[k]["wire_bytes_p95"] for k in keys),
        "per_item": [
...
if __name__ == "__main__":
    # Weights from SCOPE.md (Infra B v2): encode_cpu_ms=0.5, decode_cpu_ms=0.3
    weights = {"encode_cpu_ms": 0.5, "decode_cpu_ms": 0.3}
    # SCOPE exclusions: tiny items (error-404.json, user-profile.json)
    exclusions = ["error-404.json", "user-profile.json"]
    main(sys.argv[1], sys.argv[2], weights, exclusions=exclusions)
