...
>
> **(2) ~~Every wall figure here is optimistic~~ — FIXED 2026-07-30 (P3).** `potts.go` now reads `V(x+1,y)` where it read the uncoded `Hz(x+1,y)`, and every table here is regenerated from that legal coder. Legality cost **+4.33% to +13.71%** of the wall bill where CAE is chosen, and 0.00% below ~6,400 regions where the contour coder wins `min(CAE, contour)` anyway. The lossless total moves 11,654,978 → **11,713,104 B**, 1.51× → **1.52×** WebP.
...
| **shape coder** | exact region partition | **11,713,104** | **1.52×** |
| AVIF | `avifenc --lossless` | 11,969,137 | 1.55× |
| PNG | as delivered | 12,278,280 | 1.59× |

**1.52× WebP, 1.81× JPEG XL, just under AVIF and 4.6% under PNG.** And the shape coder is the flattered side even here: its figure is contour + colour cross-entropy with no container and no header, against real files.
...
| **walls** | 880,495 B — 0.4854 bits/edge |
...
This is worth stating plainly because it is the cleanest statement of the whole investigation's finding: **at the exact end there is no geometry to exploit, so an explicit-geometry format degenerates into a worse raster format.** The ratio is stable across sizes, and worst at native — **1.400× at 512×288, 1.352× at 960×540, 1.352× at 1920×1080, 1.510× at 3840×2160**. All four rows are now priced with `colorBytes2` (ledger item B6b, closed). They previously read 1.48/1.41/1.40/1.58 on the `colorBytesLean` pricing, so each was 0.05–0.08 too high; the shape of the curve is unchanged.
